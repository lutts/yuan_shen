...
from ..utils import ys_expect_damage
from ..syw_finder import ShengYiWu, ShengYiWu_Score, calculate_score, Syw_Combine_Desc, find_syw_combine, set_qualifier_threshold
...
from ..weapon_impl.jing_shui_liu_yong_zhi_hui import Jing_Shui_Liu_Yong_Zhi_Hui
...
class FuFu_Ch(Character, name="芙宁娜", elem_type=Ys_Elem_Type.SHUI, ming_zuo_num=6, 
                     e_level=13, q_level=13, q_energy=60):
    BASE_HP = 15307
    HEI_DAO_MULTIPLIER = 18/100
    BAI_DAO_MULTIPLIER = (18 + 25)/100

    E_MULTIPLIER = [12.58/100, 13.37/100, 14.16/100, 14.94/100, 15.73/100, 16.71/100]
    ...
    Q_MULTIPLIER = [18.25/100, 19.39/100, 20.53/100, 21.67/100, 22.81/100, 24.24/100]
    ...
    def __init__(self, weapon, ming_zuo_num=6):
        ...
        * ming_zuo_num: 命座数
        ...
        super().__init__(base_hp=FuFu_Ch.BASE_HP, weapon=weapon, crit_rate=0.242)
        ...
        self.fu_ren: Action = None
        self.xun_jue: Action = None
        self.pang_xie: Action = None

    def qi_fen_zhi_to_elem_bonus(self, qi):
        return qi * self.QI_BONUS_MULTIPLIER[self.q_idx()]
    
    def qi_fen_zhi_to_healing_bonus(self, qi):
        return qi * self.QI_HEAL_MULTIPLIER[self.q_idx()]
    
    def gu_you_tian_fu_2_bonus(self, max_hp):
        return min(max_hp / 1000 * 0.007, 0.28)
    
    def get_ge_zhe_cure_num(self, plan: ActionPlan):
        multiplier = FuFu_Ch.GE_ZHE_HEAL_MULTIPLIER[self.e_idx()]
    ...
    def add_bai_dao_damage(self, plan: ActionPlan):
        damage = self.get_max_hp() * FuFu_Ch.BAI_DAO_MULTIPLIER * (1 + self.get_normal_a_bonus())
        return plan.add_damage(damage)
    ...
    def __add_salon_member_damage(self, plan: ActionPlan, multiplier, force_add_fufu_q_bonus):
        ...
        if plan.qi_fen_zhi_supervisor.effective_qi_fen_zhi == 0 and force_add_fufu_q_bonus:
            ...
            max_hp += round(self.MAX_QI_FEN_ZHI * self.QI_HP_MULTIPLIER * FuFu_Ch.BASE_HP)
            e_bonus += self.MAX_QI_FEN_ZHI * FuFu_Ch.QI_BONUS_MULTIPLIER[self.q_level - 1]
    ...
    def add_fu_ren_damage(self, plan: ActionPlan, force_add_fufu_q_bonus):
        multiplier = FuFu_Ch.FU_REN_MULTIPLIER[self.e_level - 1]
    ...
    def add_xun_jue_damage(self, plan: ActionPlan, force_add_fufu_q_bonus):
        multiplier = FuFu_Ch.XUN_JUE_MULTIPLIER[self.e_level - 1]
        return self.__add_salon_member_damage(plan, multiplier, force_add_fufu_q_bonus)
    
    def add_pang_xie_damage(self, plan: ActionPlan, force_add_fufu_q_bonus):
        multiplier = FuFu_Ch.PANG_XIE_MULTIPLIER[self.e_level - 1]
        return self.__add_salon_member_damage(plan, multiplier, force_add_fufu_q_bonus)
...
class FuFuActionPlan(ActionPlan):
    def __init__(self, fufu_initial_state: FuFu_Ch, teammates: list[Character]):
        self.__fufu: FuFu_Ch = copy.deepcopy(fufu_initial_state)
...
class Qi_Fen_Zhi_Supervisor(Buff):
    def __init__(self, fufu: FuFu_Ch):
        self.fufu = fufu
        self.__stopped = True
...
class Hei_Fu_Cure_Teammate_Action(Hei_Fu_Cure_Action):
    ...
    def do_cure(self, plan: FuFuActionPlan):
        ...
        fufu = self.supervisor.fufu
        plan.regenerate_hp(targets=plan.get_teammates(fufu), hp=cure_num)
...
class Mang_Huang_Damage_Action(Action):
    def __init__(self, name, fufu: FuFu_Ch):
        super().__init__(name)
        self.fufu = fufu
...
def calc_score(fufu_initial_state: FuFu_Ch, 
               create_plan_func: Callable[[FuFu_Ch], FuFuActionPlan]):
    all_damage = 0
    full_six_zhan_bi = 0
    run_num = MAX_RUN_NUM
...
def scan_syw_combine(syw_combine: list[ShengYiWu], 
                     fufu_creator_func: Callable[[], FuFu_Ch],
                     check_fufu_func: Callable[[FuFu_Ch], bool]) -> FuFu_Ch:
...
def qualify_syw(score_data: ShengYiWu_Score, 
                fufu_creator_func: Callable[[], FuFu_Ch],
                check_fufu_func: Callable[[FuFu_Ch], bool],
                qualify_func):
